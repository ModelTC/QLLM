...
def smooth_ln_fcs_shift_inplace(ln, fcs, shifts):
    if not isinstance(fcs, list):
        fcs = [fcs]
    if hasattr(ln, "bias") and ln.bias is not None:
        ln.bias.sub_(shifts)
...
def smooth_fc_fc_shift_inplace(fc1, fc2, shifts=None):
    # only support for v_proj and out_proh now.
...
def smooth_fcs_fc_inplace(fc1s, fc2, scales, shifts=None):
    # only support for v_proj and out_proh now.
    if not isinstance(fc1s, list):
        fc1s = [fc1s]
    for fc1 in fc1s:
        fc1.use_temporary_parameter = False
